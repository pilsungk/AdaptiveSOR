      PROGRAM ITPTST 
C      PROGRAM ITPTST (OUTPUT,TAPE6=OUTPUT)      
C       
...
C       
C      OPEN(99,FILE=`result.out`)
C      OPEN(UNIT=6, FILE='data.test', ACCESS='SEQUENTIAL', FORM='FORMATTED')
      DOUBLE PRECISION DRELPR 
      OPEN(UNIT=6,FILE='result.out')      
C      OPEN(UNIT=8,ACCESS='SEQOUT',FILE='OUT.LPT')      
C      PI = 3.14
C      PRINT "(f6.3)", PI
C       
...
C       
      DRELPR = 1.26D-29     
      CALL TEST1D (DRELPR)  
C      CALL TEST2D (DRELPR)  
C      CALL TEST3D (DRELPR)  
C       
